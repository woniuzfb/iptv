...
function reqChannels(source,token) {
  let params;
  if (localStorage.getItem(source.name+'_verify_code')) {
    params = '?pageidx=1&pagenum=500&accesstoken='+token+'&verify_code='+localStorage.getItem(source.name+'_verify_code');
  } else {
    params = '?pageidx=1&pagenum=500&accesstoken='+token;
  }
  timeoutPromise(5000,reqData(source.list_url,params))
  .then(response => {
    if (response.ret === 0) {
      let channelCats;
